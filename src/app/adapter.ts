import { IncomingMessage, Server, ServerResponse, createServer, OutgoingHttpHeader } from "http"
...
export  class AdapterCore { 
    constructor(
        ...
        protected services: [] = []) { }
    protected async  onRequest(path: string, method: string, request: any): Promise<INeptunAdapterResponse> {
        ...
        if (!resource) return { body: "", headers: {}, status: 404 }
            ...
            {
                try {
                    const response = await resource[method](request).then((response: INeptunResponse) => response)
                    response.headers = { ...resource.GetHeaders(),...response.headers }
                    return response
                } catch (error) {

                    if(!("ERROR" in resource))
                        return { body: (error as NeptuneError).message, headers: (error as NeptuneError).headers, status: (error as NeptuneError).status }
                    else {
            ...
            else return { body: "Can not " + method + " " + path , headers: {}, status: 402 }
        }
...
export class NodeAdapter extends AdapterCore {
    ...
    constructor(
        ...
        if (this.port) {
            this.server.on("request",async (request: IncomingMessage, response: ServerResponse) => {
                 
                const path = request.url || ""
                const method = (request.method || "GET" ).toUpperCase()
                const { headers, body, status } = await this.onRequest(path, method, new NeptuneRequest(request, response, path, request.headers))
